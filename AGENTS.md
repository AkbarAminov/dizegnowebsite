...
- **Styling**: Tailwind only; colour tokens live in `src/app/globals.css` — `accent` (yellow), `canvas` (public-site ground), `surface` (admin chrome) and `panel` (admin cards), used as `bg-canvas`, `bg-panel`, … Nothing is painted pure black on purpose. Admin text uses the neutral palette. Body scroll under overlays is locked with `useLockBodyScroll`.
